...
export function useSyncSettings(): UseSyncSettingsState {
  ...
  // Parallel sync is only meaningful when at least one personal node is configured.
  // Public API rate-limits concurrent requests; personal nodes handle parallel load.
  const canEnableParallelSync = nodes.length > 0;
