...
describe('Integration: Watch-Only Wallet (xpub)', () => {
  ...
  it('network auto-detected from xpub prefix always resolves to mainnet (no testnet version bytes in xpub)', async () => {
    const { importWallet } = makeSetup();
    // bitcoin-tx-lib emits xpub (mainnet BIP32 version bytes) for all networks with purpose=44.
    // The detector maps xpub prefix → mainnet, tpub prefix → testnet.
    // Since tpub is never generated by this library at purpose=44, testnet auto-detection
    // via xpub is not possible; the key prefix always resolves to mainnet.
    const wallet = await importWallet.execute('Xpub Always Mainnet', getXpub('testnet'));
    expect(wallet.network).toBe('mainnet');
  });
